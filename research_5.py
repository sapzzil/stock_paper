# -*- coding: utf-8 -*-
"""
Created on Mon Sep 30 11:31:32 2024
...
def create_dataset(df, n=1, threshold=0.004, window_size=5):
    ...
    for i in range(len(df) - window_size - n):
        ...
        # X_window = df[['Open', 'Close', 'Volume']].iloc[i:i + window_size].values.flatten()
        X_window = df[['Open', 'Close']].iloc[i:i + window_size].values
        ...
        y_value = [0,1] if change > threshold else [1,0]
...
def evaluate_models(X_train, y_train, X_test, y_test):
    ...
    # X = X_train.reshape((X_train.shape[0], X_train.shape[1], num_features))  # LSTM 입력 형식에 맞추기 위해 3차원 변환
    X = X_train.copy()
    ...
    # 2. 모델 구성
    model = Sequential([
        LSTM(32, activation='relu', input_shape=(time_steps, num_features)),
        Dropout(0.3),
        Dense(8, activation='relu'),
        Dense(num_classes, activation='softmax')  # 분류용 sigmoid 활성화 함수
    ])
    ...
    # 4. 모델 훈련
    history = model.fit(X, y_train, epochs=20, batch_size=32, validation_split=0.2)
    
    # 5. 모델 예측
    # test = X_test.reshape((X_test.shape[0], X_test.shape[1], num_features)) 
    test = X_test.copy()
    y_pred = model.predict(test)
    y_pred = (model.predict(test) > 0.294).astype("int32")
